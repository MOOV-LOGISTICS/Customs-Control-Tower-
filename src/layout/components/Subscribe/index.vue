<template>
  <div>
    <el-popover placement="bottom" width="500" trigger="click" title="Subscribe" @show="getList"
>
      <!-- icon 展示 -->
      <el-badge class="item" slot="reference"  :hidden="unreadCount==0" :value="unreadCount" :max="99">
        <el-button style="font-size: 20px;border: 0px;background-color:transparent;" class="el-icon-message" type="primary" @click="getList"></el-button>
      </el-badge>
      ...
      <div class="popover-content"  >
        <el-scrollbar style="height:300px;">
          ...
          <el-card
            style="box-shadow: none;background-color: #e6ebf5;margin-bottom: 10px;padding-left: 8px;padding-right: 8px;"
            ...
            <!-- <el-link type="primary" target="_blank" :href="'/order/POManagement?orderNumber='+item.templateNickname">{{ item.templateNickname }}</el-link> -->
            <p class="text-ellipsis" v-html="item.templateContent"></p>
            <!-- <div class="text-ellipsis" v-html="item.templateNickname" @click="toOther('/order/POManagement?orderNumber='+item.templateNickname)"></div> -->
          </el-card>
        </el-scrollbar>
    ...
    <el-dialog v-model="clickItem" title="Subscribe" :visible.sync="messageShow" width="1200px" append-to-body>
...
<script>
import {getUnreadNotifyMessageCountBySubscribe, getUnreadNotifyMessageListBySubscribe,updateAllSubscribeNotifyMessageRead,updateNotifyMessageRead} from "@/api/system/notify/message";
...
export default {
  ...
  methods: {
    getList: function() {
      ...
      getUnreadNotifyMessageListBySubscribe(queryParams).then(response => {
