        ...
        <div class="dw-dcs-main">
          <div class="dw-dcs-main-survey-title">
            <el-row type="flex" justify="space-between" align="middle">
              ...
              <el-col :span="4">
                <el-select :value="survey.surveyState" v-model="survey.surveyState" :placeholder="$t('survey.status')" @change="surveyStateChange" >
                  <el-option key="0" :value="0" :label="$t('survey.inDesign')" ></el-option>
                  <el-option key="1" :value="1" :label="$t('survey.inCollection')" ></el-option>
                  <el-option key="2" :value="2" :label="$t('survey.endStatus')" ></el-option>
                </el-select>
              </el-col>
            </el-row>
          </div>
          <div class="dw-dcs-main-survey-step">
            <div class="dw-dcs-main-survey-step-item" style="padding: 13px 16px;">
              <el-row type="flex" justify="space-between" align="middle" >
                <el-col :span="3">
                  <el-button  class="dw-link dw-link-1" @click="buttonClickA(`/static/diaowen/design.html?surveyId=${survey.id}`)"><i class="el-icon-edit"></i>{{$t('survey.updateSurvey')}}</el-button>
                </el-col>
                <el-col :span="3" >
                  <router-link :to="`/survey/dw/survey/c/url?id=${survey.id}`" :class="{ 'dw-link-primary' : isAnswerUrl || isSurveySet || isSiteShare || isSiteComp || isAnswerWx}" class="dw-link dw-link-1" ><i class="el-icon-link"></i>{{$t('survey.answerAddress')}}</router-link>
                </el-col>
                <el-col :span="4">
                  <router-link :to="`/survey/dw/survey/d/chart?id=${survey.id}`" :class="{ 'dw-link-primary' : isSurveyChart || isAnswerData }" class="dw-link dw-link-1" ><i class="el-icon-s-data"></i>{{$t('survey.report')}}</router-link>
                </el-col>
                <el-col :span="14" style="text-align: right;">
                  <el-button type="primary" size="small" @click="buttonClickA(`/static/diaowen/design.html?surveyId=${survey.id}`)" >{{$t('survey.updateSurvey')}}</el-button>
                  <el-button size="small" @click="handlePush(`/survey/dw/survey/c/url?id=${survey.id}`)" >{{$t('survey.answerAddress')}}</el-button>
                </el-col>
              </el-row>
            </div>
...
<!--                <el-col :span="3">-->
<!--                  <router-link :to="`/survey/dw/survey/c/url?id=${survey.id}`" :class="{ 'dw-link-primary' : isAnswerUrl}" class="dw-link" ><i class="el-icon-link"></i>{{$t('survey.answerAddress')}}</router-link>-->
<!--                </el-col>-->
<!--                <el-col :span="3">-->
...
<!--                </el-col>-->
              ...
              <el-row v-show="isSurveyChart || isAnswerData">
                <el-col :span="3">
                  <router-link :to="`/survey/dw/survey/d/chart?id=${survey.id}`" :class="{ 'dw-link-primary' : isSurveyChart}" class="dw-link" ><i class="el-icon-discount"></i>Statistics</router-link>
                </el-col>
                <el-col :span="3">
                  <router-link :to="`/survey/dw/survey/d/data?id=${survey.id}`" :class="{ 'dw-link-primary' : isAnswerData}" class="dw-link" ><i class="el-icon-receiving"></i>Raw Data</router-link>
                </el-col>
            ...
            <div class="dw-dcs-main-survey-step-item dw-dcs-main-survey-step-item-status" >
              <el-row type="flex" justify="space-between" align="middle">
                <el-col :span="4">
                  <div>{{$t('survey.status')}}：
                    <el-tag v-if="survey.surveyState === 0" size="mini" >{{$t('survey.inDesign')}}</el-tag>
                    <el-tag v-else-if="survey.surveyState === 1" type="success" size="mini" >{{$t('survey.inCollection')}}</el-tag>
                    <el-tag v-else-if="survey.surveyState === 2" type="info" size="mini" >{{$t('survey.endStatus')}}</el-tag>
                    <el-tag v-else disable-transitions style="margin-left: 10px" size="mini" >{{$t('survey.errStatus')}}</el-tag>
                  </div>
                </el-col>
                <el-col :span="4">
                  <div>{{$t('survey.count')}}：{{ survey.answerNum != null ? survey.answerNum : 0 }} </div>
                </el-col>
                <el-col :span="16" style="text-align: right;">
                  {{$t('survey.createDate')}}：{{ survey.createDate }}
                </el-col>
              </el-row>
            </div>
...
export default {
  ...
  methods: {
    ...
    surveyStateChange () {
      ...
      dwSurveyUpState(this.$route.query.id, this.survey.surveyState).then((response) => {
        const httpResult = response.data
        if (httpResult.resultCode === 200) {
          this.$notify.success('问卷状态设置成功')
        } else {
          this.$notify.error('问卷状态设置失败')
        }
      })
    },
    getSurveyInfo () {
      dwSurveyInfo(this.$route.query.id).then((response) => {
