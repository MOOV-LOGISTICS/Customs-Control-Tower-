<template>
  <div class="panel-tab__content">
    ...
    <el-dialog :visible.sync="modelVisible" :title="modelConfig.title" :close-on-click-modal="false" width="400px" append-to-body destroy-on-close>
      <el-form :show-message="false"	 	 size='mini' :model="modelObjectForm"  label-width="90px" @submit.native.prevent>
      ...
      <template slot="footer">
        <el-button size="mini"   @click="modelVisible = false">{{$t('el.popconfirm.cancelButtonText')}}</el-button>
    ...
    addNewObject() {
      if (this.modelType === "message") {
        if (this.messageIdMap[this.modelObjectForm.id]) {
          return this.$notify.error("该消息已存在，请修改id后重新保存");
        }
        ...
        if (this.signalIdMap[this.modelObjectForm.id]) {
          return this.$notify.error("该信号已存在，请修改id后重新保存");
        }
