  ...
  <div style="position: relative;">
    ...
    >
      ...
      <div
        ...
        :style="{width: (leftBarWidth!==undefined)?leftBarWidth: Math.floor(parseInt(setSize.imgWidth)*47/310)+ 'px', height: barSize.height, 'border-color': leftBarBorderColor, transaction: transitionWidth}"
      >
          ...
          :style="{width: Math.floor(parseInt(setSize.imgWidth)*47/310)+ 'px',
           height: barSize.height, 
            'background-color': moveBlockBackgroundColor,
             left: moveBlockLeft, transition: transitionLeft}"
          @touchstart="start"
          @mousedown="start"
        >
          <i
  ...
  methods: {
    ...
    move: function(e) {
      ...
      if (this.status && this.isEnd === false) {
        ...
        // }
         if (move_block_left >= this.barArea.offsetWidth - parseInt(Math.floor(parseInt(this.setSize.imgWidth)*47/310) ) ) {
          move_block_left = this.barArea.offsetWidth
           - parseInt(Math.floor(parseInt(this.setSize.imgWidth)*47/310)-4) 
        }
        if (move_block_left <= 0) {
          move_block_left=0
          // move_block_left = parseInt(parseInt(this.blockSize.width) / 2)
          // this.barArea.offsetWidth - parseInt(Math.floor(parseInt(this.setSize.imgWidth)*47/310) / 2)
        }
        // 拖动后小方块的left值
      ...
      if (this.status && this.isEnd === false) {
        ...
        reqCheck(data).then(res => {
          if (res.repCode === '0000') {
            ...
            this.passFlag = true
            // this.tipWords = `${((this.endMovetime - this.startMoveTime) / 1000).toFixed(2)}s verity successfully`
            this.tipWords = `Verification Successfully`
            ...
            const captchaVerification = this.backToken 
            setTimeout(() => {
              this.tipWords = ''
