<template>
    <el-dialog title="Inbound received" @close="handleClose" class="inbound-popup" :visible.sync="formVisible" width="90%">
      <el-form ref="form" :model="form" :show-message="false" v-loading="loading">
        <el-row>
          ...
          <el-col :span="11">
            <el-form-item required prop="inboundTime" label="Inbound received date"  label-width="180px" style="margin-left: 20px;">
              <el-date-picker
        ...
        <el-table
          ...
          <el-table-column prop="productName" label="Line Description" width="200">
          </el-table-column>
...
<el-table-column  prop="PIECESInbound" width="150" label="Inbond Qty">
<template slot-scope="scope">
  <el-input  :class="getInputStrClass(scope.row,scope.row.inboundUnit)"  @change="setItem(scope.$index,scope.row.inboundUnit)" size="mini"  @input="changeBookedQty(scope.row)"
                    v-model="scope.row[scope.row.inboundUnit+'Inbound']"
                    >
                    <i slot="suffix" style="font-style:normal;margin-right: 10px; line-height: 30px; color: #1e1e1e">{{scope.row.bookedUnit}}</i>
                  </el-input>
</template>
</el-table-column>
<el-table-column  prop="CTNSInbound" width="110" label="Inbond Qty">
<template slot-scope="scope">
{{Math.ceil(scope.row['CTNS'+'Inbound']) +' '+ 'CTNS'}}
</template>
</el-table-column>
<el-table-column v-if="unitLabelsShow" prop="INNER BOXInbound" width="110" label="Inbond Qty">
<template slot-scope="scope">
{{Math.ceil(scope.row['INNER BOX'+'Inbound']) +' '+ 'INNER BOX'}}
</template>
</el-table-column>
      ...
      <div v-show="showEdit" slot="footer" class="dialog-footer">
...
import { downloadInboundTemplate,uploadInboundFiles } from "@/api/order/item";
...
import { fileLoadMixin } from '@/utils/fileLoadMixin'
  ...
  export default {
    ...
    mounted() {
      ...
      if(!this.view||this.checkPermi(['order:inbound:edit'])) {
        this.showEdit = true
      }
    ...
    getHeaderApiByOrderId(id) {
      ...
      getHeaderApiByOrderId(id).then((response) => {
        ...
        // console.log(this.form.itemResList)
        for(var item of this.form.itemResList) {
    ...
    setItem(index,inboundUnit){
      var unitL = inboundUnit
      var vm=this
      var row=vm.form.itemResList[index]
      row.inboundUnit=unitL
      row.inbound=row[inboundUnit+'Inbound']
      ...
      if(units == null||units.length==0){
        ...
        row[requestUnit+'Inbound']=row.inbound
        row['CTNS'+'Inbound']=Math.floor(row.inbound/Number(ctnsUnit.conversionFactor))
          if(row.inbound%Number(ctnsUnit.conversionFactor)>0&&piecUnit!=null){
            var shengyu=row.inbound%Number(ctnsUnit.conversionFactor)
            row['CTNS'+'Inbound']= Math.floor(row.inbound/Number(ctnsUnit.conversionFactor))
            row['INNER BOX'+'Inbound']= Math.floor(shengyu/piecUnit.conversionFactor)
          }
                ...
                if (tabJson && tabJson.length > 0) {
                    ...
                    for(var i=0;i<this.xlsxJson[0].sheet.length;i++){
                        ...
                        if(item['SKU(Article)']){
                            ...
                            console.log(index)
                            if(index != -1){
                                vm.form.itemResList[index]['inbound'] = item['Inbound Quantity']
                                vm.form.itemResList[index]['inboundUnit'] = item.inboundUnit
                            }else{
    ...
    downloadTemplate(){
      downloadInboundTemplate(this.form.rule).then((res)=>{
        this.$download.excel(res, "inbound_template.xlsx");
      })
    },
    submit() {
			var vm = this
      this.$refs["form"].validate(valid => {
        ...
        vm.form.inboundJson = JSON.stringify(vm.form.itemResList)
        ...
        formData.append("inboundJson", vm.form.inboundJson);
        ...
        vm.loading = true
        uploadInboundFiles(formData).then(response => {
          vm.fileList = [];
          vm.deletcFileList = [];
          vm.$emit('close-inbound-popup');
        });
