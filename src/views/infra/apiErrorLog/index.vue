  ...
  <div class="app-container" style="width:90%;margin:0 auto;">
    ...
    <el-form :show-message="false"	 	 size='mini' :model="queryParams" ref="queryForm"  :inline="true" v-show="showSearch" label-width="120px">
      ...
      <el-form-item label="异常时间" prop="exceptionTime">
        <el-date-picker v-model="queryParams.exceptionTime"  value-format="yyyy-MM-dd HH:mm:ss" type="daterange"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button size="mini"  type="warning" plain icon="el-icon-download"  @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['infra:api-error-log:export']">{{$t('search.export')}}</el-button>
      </el-col>
    ...
    <el-dialog title="API 异常日志详细" :visible.sync="open" width="1280px" append-to-body>
      <el-form :show-message="false"	 	 size='mini' ref="form" :model="form" label-width="100px">
        <el-row :gutter="30">
          <el-col :span="24">
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有API 错误日志数据项?').then(() => {
        ...
        this.$download.excel(response, 'API 错误日志.xlsx');
