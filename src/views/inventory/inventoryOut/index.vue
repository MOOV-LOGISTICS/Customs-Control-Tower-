<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="" prop="outboundTime">
        <el-date-picker v-model="queryParams.outboundTime" style="width: 240px" value-format="yyyy-MM-dd HH:mm:ss" type="daterange"
                        range-separator="-" start-placeholder="Outbound data" end-placeholder="Outbound data" :default-time="['00:00:00', '23:59:59']" />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="Outbound Qty" align="center" prop="qty" />
...
<script>
import { createInventoryOut, updateInventoryOut, deleteInventoryOut, getInventoryOut, getInventoryOutPage, exportInventoryOutExcel
,downloadInventoryOutTemplate } from "@/api/order/inventoryOut";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/admin-api/order/inventory-out/import",
      },
      uploadError:{
        title: "Errors",
        dialogVisible: false,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getInventoryOutPage(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      this.reset();
      const id = row.id;
      getInventoryOut(id).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        // 添加的提交
        createInventoryOut(this.form).then(response => {
          this.$modal.msgSuccess("Success");
          this.open = false;
          this.getList();
        });
    ...
    handleExport() {
      ...
      this.$modal.confirm(this.$t("purchaseOrder.confirmThatAllDataItemsAreExported")).then(() => {
          ...
          return exportInventoryOutExcel(params);
        }).then(response => {
          this.$download.excel(response, "smartMOOV_Outbound"+this.parseTimeToZ2(new Date())+".xlsx");
    ...
    /** 下载模板操作 */
    importTemplate() {
      downloadInventoryOutTemplate().then((response) => {
        this.$download.excel(response, "Outbound.xlsx");
      });
    },
    // 文件上传中处理
