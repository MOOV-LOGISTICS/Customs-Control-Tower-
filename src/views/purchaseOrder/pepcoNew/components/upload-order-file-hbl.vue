<template>
  <el-dialog :title="title" :visible.sync="dialogVisible" width="70%" append-to-body>
    <div>
      <!-- Upload Button -->
      <el-button v-if="isEdit && !reUploadHblFile" type="primary" icon="el-icon-upload2" @click="showUploadDialog">Upload</el-button>
      <el-button v-if="reUploadHblFile"  type="primary" style=" margin-bottom: 10px;" icon="el-icon-upload2" @click="showUploadDialog">Upload</el-button>
        ...
        <el-table-column label="Action" width="250px" align="center">
          <template #default="scope">
            ...
            <template v-if="scope.row.typeCode !== '15' ">
              ...
              <el-link>
                <el-button v-if="isEdit && !reUploadHblFile"  type="danger" icon="el-icon-delete" @click="handleDelete(scope.row)"></el-button>
              </el-link>
              <el-link>
                <el-button v-if="reUploadHblFile"  type="danger" icon="el-icon-delete" @click="handleDelete(scope.row)"></el-button>
              </el-link>
    ...
    <el-table-column prop="result" label="Result" min-width="140px" align="center">
      <template #default="scope">
        ...
        <div style="float:right">
          ...
          <el-button size="mini" v-if="(isEdit && showUpload )|| reUploadHblFile" type="primary"  @click="submitUploadForm">{{ $t("purchaseOrder.confirm") }}</el-button>
          <!-- <el-button size="mini" v-if="reUploadHblFile" type="primary"  @click="submitUploadForm">{{ $t("purchaseOrder.confirm") }}</el-button> -->

          <!-- :disabled="uploadForm.typeCode == '03'&&(uploadForm.showTip==1||uploadSet.notMatch)&&task.country != 'BD'" -->
          <el-button size="mini" v-if="!showUpload && !reUploadHblFile"  type="primary" @click="reSubmitForm">{{ $t("purchaseOrder.confirm") }}</el-button>
...
import { updateTask, updatePreAlertTask, reSaveFile, reUploadFileByHbl, getSingleTask } from '@/api/order/task'
...
export default {
  ...
  methods: {
    ...
    openDialog(task, activeTask, currentSeq) {
      ...
      // 判断 如果有权限 且pre alert节点已经完成，就打开edit =true
      if (task.taskCode === 'Release_HBL_and_Invoice_to_Shipper' && task.status == 2) {
        // 调用接口，判断传参 task.orderId,"Pre_Alert"
        getSingleTask(this.task.orderNumber, 'Pre_Alert').then(res => {
          console.log('Pre_Alert', res.data)
          if (res.data && res.data.status == 2) {
            // 再判断权限 v-hasPermi="['pepco:task:upload-hbl']"
            if (this.$auth.hasPermi('pepco:task:upload-hbl')) {
              this.reUploadHblFile = true
            }
          }
        })
      }
    ...
    reSubmitForm() {
      ...
      this.uploadSet.url = process.env.VUE_APP_BASE_API + '/admin-api/order/task/pepco/order-file/hbl-re-save'
      ...
      if (this.originalFileList[0].name == this.fileList[0].name) {
        ...
        reUploadFileByHbl(null, this.uploadForm).then(res => {
      ...
      // 文件有变化时，也使用 reUploadFileByHbl 处理
      console.log('文件有变化，执行文件上传')
      if (uploadFiles && uploadFiles.length > 0) {
        const file = uploadFiles[0].raw  // 获取原始文件对象
        console.log('上传的文件', file)
        reUploadFileByHbl(file, this.uploadForm).then(res => {
          console.log('返回结果', res)
          if (res.code == 0) {
