  ...
  <div>
    ...
    <el-table :data="tableData" v-loading="tableLoading">
      ...
      <el-table-column label="Action" width="220px" align="center">
        <template #default="scope">
          ...
          <el-link>
            <el-button v-if="isEdit" style="margin-left: 5px;" type="primary" icon="el-icon-edit" @click="handleEdit(scope.row)"></el-button>
          </el-link>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="uploadSet.title" :visible.sync="showDialog" width="600px" append-to-body v-loading="uploadSet.loading" :before-close="uploadClose">
      <el-form label-width="150px" ref="uploadFormRef" :model="uploadForm" :show-message="false" :rules="rules">
        ...
        <el-form-item :label="uploadForm.typeCode=='19'?'HAWB Number':uploadForm.typeCode=='20'?'MAWB Number':$t('document.documentNumber')" prop="documentNumber">
        ...
        <el-form-item :label="$t('document.documentFile')">
          <el-upload
            ref="uploadFileRef"
            :limit="1"
            :accept="uploadForm.typeCode === '16' || uploadForm.typeCode === '17' ? 'image/*, .jpeg, .jpg, .png, .gif, .doc, .docx, .pdf, .xls, .xlsx' : '.doc, .docx, .pdf, .xls, .xlsx .csv'"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        blType: [
          {
            required: () => this.uploadForm.typeCode === '20'||this.uploadForm.typeCode === '19', // 仅在 typeCode=20（MAWB）时必填
  ...
  methods: {
    ...
    submitUploadForm() {
      ...
      this.uploadUrl = process.env.VUE_APP_BASE_API + '/admin-api/order/task/pepco/order-file/shipment-upload'
    ...
    handleFileChange(file, fileList) {
      ...
      const acceptType = ['doc', 'docx', 'ppt', 'pptx', 'rar', 'txt', 'xls', 'xlsx', 'zip', 'pdf', 'jpeg', 'gif', 'jpg', 'png', 'csv']
    ...
    reSubmitForm() {
      ...
      this.uploadUrl = process.env.VUE_APP_BASE_API + '/admin-api/order/task/pepco/order-file/re-save'
