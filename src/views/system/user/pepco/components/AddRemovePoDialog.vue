...
<script>
import { getOrder4Factory, getOrder4FactorySelected } from '@/api/order/header'
import { createUserOrder } from '@/api/system/userOrder'
...
export default {
  ...
  methods: {
    ...
    async getSelectedBookings() {
      await getOrder4FactorySelected({ userId: this.userId }).then(response => {
    ...
    submit() {
      if (this.selectedTable.length === 0) {
        ...
        createUserOrder(data).then(response => {
          this.$message({
            message: 'Success',
            type: 'success'
          })
