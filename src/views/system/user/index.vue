  ...
  <div class="app-container" style="width:100%; margin: 0 auto">
    <el-card style="padding-top: 20px;">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="6" :xs="24">
          <div class="head-container">
            <el-input v-model="deptName" :placeholder="$t('role.pleaseEnterTheDepartmentName')" clearable
              prefix-icon="el-icon-search" style="margin-bottom: 20px;margin-left: 10px;" />
          </div>
          <div class="head-container">
            <el-tree :style="{ height: height }" style="overflow: auto" :data="deptOptions" :props="defaultProps"
        ...
        <el-col :span="18" :xs="24">
          <el-form :show-message="false" size="mini" :model="queryParams" ref="queryForm" v-show="showSearch"
            label-width="120px">
            <el-row :gutter="20">
              <el-col :span="6">
                <el-form-item prop="nickname">
                  <el-input placeholder="Nick Name" v-model="queryParams.nickname" clearable
                    @keyup.enter.native="handleQuery" />
                </el-form-item>
              </el-col>
              <el-col :span="6">
                <el-form-item prop="email">
                  <el-input placeholder="Email Address" v-model="queryParams.email" clearable
                            @keyup.enter.native="handleQuery" />
                </el-form-item>
              </el-col>
              <el-col :span="6">
              ...
              <el-col :span=4>
                ...
                <el-form-item>
                  <el-button @click="handleQuery" type="primary">Search<i class="el-icon-search el-icon--right"></i>
                  </el-button>
          ...
          <el-row :gutter="10" class="mb8" style="margin-right: 20px;">
            <el-col :span="1.5">
              <el-button size="mini" type="primary" plain icon="el-icon-plus" @click="handleAdd"
                v-hasPermi="['system:user:create']">{{ $t("search.addNew") }}
              </el-button>
            </el-col>
            <el-col :span="1.5" v-if="needSubAdminUser">
              <el-button size="mini" type="primary" plain icon="el-icon-plus" @click="handleAddSubAdmin"
                v-hasPermi="['system:user:create']">{{ $t("search.addSubAdmin") }}
              </el-button>
            </el-col>
            <el-col :span="1.5">
              <el-button size="mini" type="info" icon="el-icon-upload2" @click="handleImport"
                v-hasPermi="['system:user:import']">{{ $t("search.import") }}
              </el-button>
            </el-col>
            <el-col :span="1.5">
              <el-button size="mini" type="warning" icon="el-icon-download" @click="handleExport"
                :loading="exportLoading" v-hasPermi="['system:user:export']">{{ $t("search.export") }}
              </el-button>
            </el-col>
          ...
          <el-table v-loading="loading" :data="userList">
            <el-table-column label="Nick Name" align="center" key="nickname" prop="nickname" v-if="columns[2].visible"
              width="150" />
            <el-table-column label="Email" align="center" key="email" prop="email" v-if="columns[3].visible"
              width="150" />
            <el-table-column label="Party" align="center" key="deptName" prop="dept.name" v-if="columns[4].visible"
              width="150" />

            <el-table-column label="Status" key="status" v-if="columns[6].visible" align="center">
              <template v-slot="scope">
                <el-switch :disabled="!userUpdate" v-model="scope.row.status" :active-value="0" :inactive-value="1"
            ...
            <el-table-column label="Role" key="role" align="left" min-width="200px">
              <template v-slot="scope">
                <el-tag style="margin-right: 10px;margin-bottom: 4px;margin-top: 4px;"
                  v-for="roleId in scope.row.roleIds" :key="roleId">{{ roleSet[roleId] }}
                </el-tag>
              </template>
            </el-table-column>

            <!-- 是否是子管理员 -->
            <el-table-column label="Sub Admin" key="subAdmin" v-if="needSubAdminUser && columns[8].visible" align="center" min-width="100px">
              <template v-slot="scope">
                <el-switch :disabled="!needSubAdminUser" v-model="scope.row.subAdmin"
                  @change="handleSubAdminChange(scope.row)" />
              </template>
            </el-table-column>
            <!-- 公司类型 -->
            <el-table-column v-if="needSubAdminUser && columns[9].visible" label="CompanyType" key="coType" align="left" min-width="200px">
              <template v-slot="scope">
                <div v-if="scope.row.loadCompanyType">
                  <el-tag style="margin-right: 10px;margin-bottom: 4px;margin-top: 4px;"
                    v-for="coTypeId in scope.row.companyTypeIds" :key="coTypeId">{{ coTypeSet[coTypeId] }}
                  </el-tag>
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="Operation" align="center" width="160" class-name="small-padding fixed-width"
              ...
              <template v-slot="scope">
                <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                  v-hasPermi="['system:user:update']">Edit
                </el-button>
                <el-dropdown @command="(command) => handleCommand(command, scope.$index, scope.row)" v-hasPermi="[
                  'system:user:delete',
                  'system:user:update-password',
                  'system:permission:assign-user-role',
                  'assign:user:reports',
                ]">
                  <el-button size="mini" type="text" icon="el-icon-d-arrow-right">More
                  </el-button>
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item command="handleReport" type="text" icon="el-icon-s-marketing"
                      v-hasPermi="['assign:user:reports']">Reports
                    </el-dropdown-item>
                    <el-dropdown-item command="handleDelete" v-if="scope.row.id !== 1" type="text" icon="el-icon-delete"
                      v-hasPermi="['system:user:delete']">Delete
                    </el-dropdown-item>
                    <el-dropdown-item command="handleResetPwd" type="text" icon="el-icon-key"
                      v-hasPermi="['system:user:update-password']">Reset Password
                    </el-dropdown-item>
                    <el-dropdown-item command="handleRole" type="text" icon="el-icon-circle-check"
                      v-hasPermi="['system:permission:assign-user-role']">User Role
                    </el-dropdown-item>
                    <el-dropdown-item command="handleSchedule" type="text" icon="el-icon-circle-check"
                      v-hasPermi="['system:schedule:assign-user-schedule']">User Schedule Carrier
                    </el-dropdown-item>
                    <el-dropdown-item command="handleParty" type="text" icon="el-icon-circle-check"
                      v-hasPermi="['system:permission:assign-user-role']">Assign Party
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </template>
      ...
      <el-dialog :title="title" :visible.sync="open" width="75%" append-to-body>
        <el-form :show-message="false" size="mini" ref="form" :model="form" :rules="isPepcoNeed ? rulesPepco : rules" label-width="150px">
          <el-row :gutter="30">
            <el-col :span="12">
          ...
          <el-row :gutter="30">
            <el-col :span="12">
              <el-form-item label="Party" prop="deptId">
                <el-select v-model="form.deptId" filterable clearable>
                  <el-option v-for="dept in deptOptions" :value="dept.id" :key="dept.id" :label="dept.name">
                    <el-tooltip placement="top" :open-delay="100" popper-class="company-full-width-tooltip-add-user">
                        <div slot="content">
                          <p>Address: {{ dept.address }}</p>
                          <p>Phone: {{ dept.phone }}</p>
                          <p>Fax: {{ dept.fax }}</p>
                          <p>Email: {{ dept.email }}</p>
                          <p>Enterprise: {{ dept.companyUsci }}</p>
                        </div>
                        <span class="company-option-with-tooltip">{{ dept.name }}</span>
                      </el-tooltip>
                  </el-option>
                </el-select>
              </el-form-item>
            ...
            <!--pepco租户不需要 edwin-->
            <el-col :span="12" v-if="!isPepcoNeed">
              <el-form-item label="Post">
                <el-select v-model="form.postIds" multiple filterable clearable placeholder="Select">
            ...
            <!--pepco租户不需要 edwin-->
            <el-col :span="12" v-if="!isPepcoNeed">
              <el-form-item label="Customer" prop="customers">
                <el-select collapse-tags v-model="form.customers" multiple clearable filterable placeholder="Select">
                  <el-option v-for="dept in customerOptions" :value="dept.id" :key="dept.id"
          ...
          <el-row :gutter="30">
            ...
            <!--是否是子管理员-->
            <!-- <el-col v-if="form.id == null" :span="6">
                <el-form-item label="Sub Admin" prop="subAdmin">
                  <el-switch :disabled="!SubAdminUserUpdate" v-model="form.subAdmin"
                    style="margin-left: 20px;"></el-switch>
                </el-form-item>
              </el-col> -->
            <!-- 公司类型-->
            <el-col v-if="needSubAdminUser && ((form.subAdmin && form.id == null) || form.subAdmin)">
        ...
        <el-upload ref="upload" :limit="1" accept=".xlsx, .xls" :headers="upload.headers"
          ...
          <div class="el-upload__tip text-center" slot="tip">
            ...
            <el-link type="primary" :underline="false" style="font-size: 12px; vertical-align: baseline"
              @click="importTemplate">{{ $t("purchaseOrder.downloadingTheTemplate") }}
            </el-link>
          </div>
        </el-upload>
        <div slot="footer" class="dialog-footer">
        ...
        <el-form :show-message="false" size="mini" :model="form" label-width="120px">
          ...
          <el-form-item label="Carrier">
            <el-select v-model="form.scheduleIds" multiple placeholder="Select">
              <el-option v-for="item in scheduleOptions" :key="parseInt(item.id)" :label="item.label"
                :value="parseInt(item.id)"></el-option>
            </el-select>
          </el-form-item>
        </el-form>
      ...
      <el-dialog title="Assign Reports" :visible.sync="openReport" width="70%" append-to-body>
        <el-form :show-message="false" size="mini" :model="form" label-width="120px">
          ...
          <el-form-item label="Report">
            <el-cascader style="width: calc(100%);" filterable v-model="form.reportIds" size="medium"
              :options="userReportData" :props="props" show-all-levels="false" popper-class="report-css"
...
import { listSimpleDepts,listDetailDepts } from "@/api/system/dept";
...
export default {
  ...
  data() {
    return {
      ...
      columnsWithSubAdmin: [
        ...
        { key: 6, label: `Status`, visible: true },
        { key: 7, label: `Create Time`, visible: true },
        { key: 8, label: `Sub Admin`, visible: true },
        { key: 9, label: `Company Type`, visible: true }
      ],
      // 表单校验
      rules: {
      ...
      rulesPepco: {
        nickname: [{ required: true, message: "The nickname cannot be empty", trigger: "blur" }],
        // password: [{ required: true, message: "The password cannot be empty", trigger: "blur" }],
        deptId: [{ required: false, message: "", trigger: "blur" }],
        email: [
          {
            required: true,
            type: "email",
      ...
      userUpdate: false,
      // 是否需要子管理员这个功能，目前应该是pepco租户下才能修改
      needSubAdminUser: false,
      // 是否是pepco需要的
      isPepcoNeed: false,
      // 获取company type
      systemPartyRole: [],
  ...
  created() {
    ...
    console.log('get local tenantID');
    ...
    if (tenantID) {
      ...
      if (isMatched) {
        this.needSubAdminUser = true
        this.isPepcoNeed = true
      } else {
        this.needSubAdminUser = false
        this.isPepcoNeed = false
      }
    }

    this.systemPartyRole = getDictDatas(DICT_TYPE.SYSTEM_PARTY_ROLE)
    // 通过id获得party type value
    if (this.systemPartyRole && this.systemPartyRole.length > 0) {
      for (var data of this.systemPartyRole) {
        this.coTypeSet[data.id] = data.value
      }
    }
    // console.log("==============================================")
  ...
  computed: {
  columns() {
    return this.needSubAdminUser ? this.columnsWithSubAdmin : this.columnsWithoutSubAdmin;
  }
},
  methods: {
    generateRandomPassword(length) {
      // 确保生成的密码至少包含四种字符类型（大写字母、小写字母、数字和特殊符号）
      if (length < 4) {
    ...
    getList() {
      ...
      listUser(this.queryParams).then((response) => {
        this.userList = response.data.list.map(user => {
          // 设置 loadCompanyType 是否加载公司类型信息 edwin
          user.loadCompanyType = user.subAdmin
          return user
        });
    ...
    handleAddSubAdmin() {
      this.reset();
      this.form.subAdmin = true
      // 获得下拉数据
      this.getTreeselect();
      // 打开表单，并设置初始化
      this.open = true;
      this.title = "Add Sub Admin";
      if (this.form.id == null) {
        this.form.password = this.generateRandomPassword(12);
      }
      console.log(this.form.password);
    ...
    handleUpdate(row) {
      ...
      getUserInfo(id).then((response) => {
        ...
        // // 将用户的公司类型直接赋值
        // response.data.companyTypeIds = response.data.companyTypeIds;
    ...
    /** 重置密码按钮操作 */
    /** Carrie 2025-11-06  重置密码按钮操作交互：取消弹窗直接获取接口 */
    handleResetPwd(row) {
      // this.$prompt('Please input"' + row.nickname + '"new password', "Tips", {
      this.$confirm("Are you sure you want to reset the user's password?", "Tips", {
    ...
    handleReport(row) {
      ...
      //获取用户报表
      getUserReportsById(row.id).then(response => {
        this.form.reportIds = response.data
      })
    ...
    handleSchedule(row) {
      ...
      listSchedule().then((response) => {
        // 处理 scheduleOptions 参数
        this.scheduleOptions = [];
        this.scheduleOptions.push(...response.data);

      });
      // 获得角色拥有的菜单集合
      listUserSchedules(id).then((response) => {
        // 设置选中
...
.company-full-width-tooltip-add-user{
