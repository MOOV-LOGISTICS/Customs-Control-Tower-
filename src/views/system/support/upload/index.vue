  ...
  <div class="app-container">
    <el-card class="wait-task-user-box-card" style="padding-left: 10px;">
      <el-tabs class="maps" v-model="activeName">
        <el-tab-pane label="Manual" name="Manual">
          ...
          <div class="tablediv">
            ...
            <el-table v-loading="loading" :data="list" :height="tableHeight">
              <el-table-column label="Manual Name" align="center" prop="name" />
        ...
        <el-tab-pane label="Smart Video" name="Smart Video">
          <!-- 搜索工作栏 -->
          <el-form :model="queryParams1" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
            <div style="float: left;padding-top: 10px;">
              <el-row>
                <div style="float: left">
                  <svg @click="handleUpload1" style="cursor: pointer;" width="30" height="30" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <mask id="icon-21f8f28d7f10b972" maskUnits="userSpaceOnUse" x="0" y="0" width="48" height="48" style="mask-type: alpha">
                      <path d="M48 0H0V48H48V0Z" fill="#004F7C" />
                    </mask>
                    <g mask="url(#icon-21f8f28d7f10b972)">
          ...
          <!-- 列表 -->
          <!-- <el-empty v-if="list1.length==0" style="height: tableHeight;" ></el-empty> -->
          <el-table v-loading="loading" :data="list1" :height="tableHeight">
            <el-table-column label="Video Name" align="center" prop="name" />
            ...
            <el-table-column label="Hidden" align="center" class-name="small-padding fixed-width">
              <template v-slot="scope">
                <el-switch v-model="scope.row.dohidden" @change="switchChange1($event,scope.row)"></el-switch>
              </template>
            </el-table-column>

            <el-table-column label="Operation" align="center" class-name="small-padding fixed-width">
              <template v-slot="scope">
                <!-- <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate1(scope.row)">Edit</el-button> -->
                <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)">Delete</el-button>
          ...
          <!-- 分页组件 -->
          <pagination v-show="total1 > 0" :total="total1" :page.sync="queryParams1.pageNo" :limit.sync="queryParams1.pageSize" @pagination="getList1" />
        </el-tab-pane>
      </el-tabs>
    </el-card>
    ...
    <el-dialog :title="uploadDocument.title" :visible.sync="uploadDocument.open" width="400px" append-to-body>
      <el-form ref="uploadForm" :show-message="false" size="mini">
        <el-row style="padding: 10px;">
          <el-col :span="7" style="margin-top: 3px;">Manual Name:</el-col>
          <el-col :span="17">
            <el-input v-model="uploadDocument.name" placeholder="Manual Name" />
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-upload
              ...
              multiple
              :action="uploadDocument.url + '?type=support_manual&name=' + uploadDocument.name +'&id='+uploadDocument.id+'&file_id='+uploadDocument.fileId"
    ...
    <!-- 对话框1(添加 / 修改) -->
    <el-dialog :title="uploadDocument1.title" :visible.sync="uploadDocument1.open" width="400px" append-to-body>
      <el-form ref="uploadForm1" :show-message="false" size="mini">
        <el-row style="padding: 5px;">
          <el-col :span="7" style="margin-top: 3px;">Video Name:</el-col>
          <el-col :span="17">
            <el-input v-model="uploadDocument1.name" placeholder />
          </el-col>
        </el-row>
        <el-row style="padding: 5px;">
          <el-col :span="7" style="margin-top: 3px;">Category:</el-col>
          <el-col :span="17">
            <el-input v-model="uploadDocument1.category" placeholder />
          </el-col>
        </el-row>
        <el-row style="padding: 5px;">
          <el-col :span="7" style="margin-top: 3px;">Sort:</el-col>
          <el-col :span="17">
            <el-input oninput="value=value.replace(/[^0-9.]/g,'')" v-model="uploadDocument1.sort" placeholder />
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-upload
              ref="uploadDocument1"
              :limit="1"
              :headers="uploadDocument1.headers"
              multiple
              :action="uploadDocument1.url + '?type=support_video&name=' + uploadDocument1.name +'&category='+uploadDocument1.category+'&sort='+uploadDocument1.sort +'&id='+uploadDocument.id+'&file_id='+uploadDocument.fileId"
              :disabled="uploadDocument1.isUploading"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini" type="primary" @click="submitFileForm1">
          {{
          $t("purchaseOrder.confirm")
          }}
        </el-button>
        <el-button size="mini" @click="uploadDocument1.open = false">
          {{
          $t("purchaseOrder.cancel")
          }}
        </el-button>
...
export default {
  ...
  methods: {
    ...
    getList1() {
      ...
      getSupportFilePage(this.queryParams1).then(response => {
    ...
    handleUpload1() {
      this.uploadDocument1.id = ''
      this.uploadDocument1.name = ''
      this.uploadDocument1.fileId = ''
      this.uploadDocument1.open = true
    },
    // 文件上传中处理
    ...
    handleFileUploadProgress1(event, file, fileList) {
      this.uploadDocument1.isUploading = true
    },
    // 文件上传成功处理
    ...
    handleFileSuccess1(response, file, fileList) {
      ...
      this.uploadDocument1.open = false
      this.uploadDocument1.isUploading = false
      this.$refs.uploadDocument1.clearFiles()
      this.getList1()
    },
    // 提交上传文件
    submitFileForm() {
    ...
    submitFileForm1() {
      ...
      this.$refs.uploadDocument1.submit()
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      const id = row.id
      getSupportFile(id).then(response => {
        this.uploadDocument.id = response.data.id
        this.uploadDocument.name = response.data.name
        this.uploadDocument.open = true
    ...
    handleUpdate1(row) {
      const id = row.id
      getSupportFile(id).then(response => {
        this.uploadDocument1.id = response.data.id
        this.uploadDocument1.name = response.data.name
        this.uploadDocument1.sort = response.data.sort
        this.uploadDocument1.category = response.data.category
        this.uploadDocument1.open = true
        this.uploadDocument1.title = 'Update'
      })
    }
  }
}
