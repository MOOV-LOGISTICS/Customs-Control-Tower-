    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button size="mini"  type="primary" plain icon="el-icon-plus"  @click="handleAdd"
                   v-hasPermi="['system:tenant-package:create']">{{ $t('search.addNew') }}</el-button>
      </el-col>
      ...
      <el-table-column label="" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini"   type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['system:tenant-package:update']">{{ $t('search.modify') }}</el-button>
          <el-button size="mini"   type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['system:tenant-package:delete']">{{ $t('search.delete') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="50%" append-to-body>
      <el-form :show-message="false"	 	 size='mini' ref="form" :model="form" :rules="rules" label-width="140px">
        ...
        <el-form-item label="Menu permissions">
          <el-checkbox v-model="menuExpand" @change="handleCheckedTreeExpand($event)">{{$t('search.expandCollapse')}}</el-checkbox>
          <el-checkbox v-model="menuNodeAll" @change="handleCheckedTreeNodeAll($event)">Select All/Select None</el-checkbox>
          <el-tree class="tree-border" :data="menuOptions" show-checkbox ref="menu" node-key="id"
                   :check-strictly="menuCheckStrictly" empty-text="Loading..." :props="defaultProps"></el-tree>
        </el-form-item>
        <el-form-item label="Status" prop="status">
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      const id = row.id;
      this.open = true;
      this.title = "Update Customer Plan";
      // 获得菜单列表
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除Customer Plan编号为"' + id + '"的数据项?').then(function() {
