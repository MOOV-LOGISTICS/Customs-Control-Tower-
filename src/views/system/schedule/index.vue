  ...
  <div class="app-container" style="width:100%; margin: 0 auto">
    ...
    >
      ...
      <el-form-item label="From Unit" prop="fromUnit">
        <el-input
          v-model="queryParams.fromUnit"
          placeholder="Please input from unit"
          clearable
      ...
      <el-form-item label="To Unit" prop="toUnit">
        <el-input
          ...
          placeholder="Please input to unit"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="Conversion Factor" prop="conversionFactor">
        <el-input
          ...
          placeholder="Please input conversion factor"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:unit-conversion:create']"
          >{{ $t("search.addNew") }}</el-button
        >
      </el-col>
      ...
      <el-col :span="1.5">
        <el-button
          ...
          v-hasPermi="['system:unit-conversion:export']"
    ...
    <el-table v-loading="loading" :height="height" :data="list">
      ...
      <el-table-column label="Product Code" align="left" prop="productId" width="150">
        <template v-slot="scope">
          {{ productMap[scope.row.productId].code }}
        </template>
      </el-table-column>
      ...
      >
        <template v-slot="scope">
          <el-button
            ...
            v-hasPermi="['system:unit-conversion:update']"
            >{{ $t("search.modify") }}</el-button
          >
          <el-button
            size="mini"
            type="text"
...
import {
  ...
  getUnitConversion,
  getUnitConversionPage,
  exportUnitConversionExcel,
  importTemplate,
} from "@/api/system/unitConversion";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        productId: [
          { required: true, message: "productId cannot be empty", trigger: "blur" },
        ],
        fromUnit: [
          { required: true, message: "fromUnit cannot be empty", trigger: "blur" },
        ],
  ...
  methods: {
    ...
    getProductList() {
      ...
      var vm = this;
      // 执行查询
      getProductList({ ids: [].join(",") }).then((response) => {
