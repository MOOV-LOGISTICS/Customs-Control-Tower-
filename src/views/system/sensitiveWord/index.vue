<template>
  <div class="app-container" style="width:90%;margin:0 auto;">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button size="mini"  type="primary" plain icon="el-icon-plus"  @click="handleAdd"
                   v-hasPermi="['system:sensitive-word:create']">{{ $t('search.addNew') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button size="mini"  type="warning" plain icon="el-icon-download"  @click="handleExport"
                   :loading="exportLoading" v-hasPermi="['system:sensitive-word:export']">{{$t('search.export')}}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button size="mini"  type="success" plain icon="el-icon-document-checked"  @click="handleTest">测试</el-button>
      </el-col>
    ...
    <el-dialog title="检测敏感词" :visible.sync="testOpen" width="500px" append-to-body>
      <el-form :show-message="false"	 	 size='mini' ref="testForm" :model="testForm" :rules="testRules" label-width="120px">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini"  type="primary" @click="submitTestForm">检 测</el-button>
        <el-button size="mini"  @click="cancelTest">{{$t('el.popconfirm.cancelButtonText')}}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
