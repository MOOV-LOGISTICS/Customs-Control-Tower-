<template>
  <div class="app-container">
    ...
    <div class="upload-top">
      <el-upload
        v-hasPermi="['warehouse:outbound:upload-data']"
        class="upload-btn"
        action="#"
    ...
    <div >
        <el-form :show-message="false" size="mini" :model="queryParams" ref="queryForm"  :inline="true" label-width="130px">
            ...
            <el-form-item label="Outbound SO#" prop="outboundNo">
                <el-input v-model="queryParams.outboundNo" placeholder="outbound DN#"
                 @blur="queryParams.outboundNo = $event.target.value.trim()" clearable @keyup.enter.native="handleQuery"/>
            </el-form-item>
    ...
    >
            ...
            <template slot-scope="scope">
                <div style="max-width: 90%;" v-if="scope.row">
                    <el-tabs type="border-card">
                        ...
                        <el-tab-pane label="Consignee Info">
                          <el-table :data="[scope.row]" size="mini" style="width: 100%;">
                            <el-table-column prop="consigneeName" label="Consignee name" min-width="150px">
                                <template slot-scope="innerScope">
                                    <span>{{ innerScope.row.consigneeName || '-' }}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="consigneePhone" label="Consignee phone" min-width="150px">
                                <template slot-scope="innerScope">
                                    <span>{{ innerScope.row.consigneePhone || '-' }}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="consigneeEmail" label="Consignee email" min-width="200px">
                                <template slot-scope="innerScope">
                                    <span>{{ innerScope.row.consigneeEmail || '-' }}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="consigneeAddress" label="Consignee address" min-width="300px">
                                <template slot-scope="innerScope">
                                    <span>{{ innerScope.row.consigneeAddress || '-' }}</span>
                                </template>
                            </el-table-column>
                          </el-table>
                        </el-tab-pane>
        ...
        <el-table-column prop="outboundNo" label="Outbound SO#" min-width="150px"></el-table-column>
        ...
        <el-table-column prop="outboundDate" label="Outbound Date" :formatter="formatDate" min-width="150px"></el-table-column>

        <el-table-column prop="shippingType" label="Shipping Type" min-width="150px"></el-table-column>
        <el-table-column prop="containerNumber" label="Container number" min-width="150px"></el-table-column>
        <el-table-column prop="sealNumber" label="Seal number" min-width="150px"></el-table-column>
        ...
        <el-table-column prop="actualTotalCbmQty" label="Total CBM Variance%" min-width="150px">
            <template slot-scope="scope">
                <!-- 计算 Total CBM Variance% -->
                {{ ((scope.row.actualTotalCbmQty?0:scope.row.actualTotalCbmQty) - scope.row.asnTotalCbmQty) / scope.row.asnTotalCbmQty * 100 }}
            </template>
        </el-table-column>

        <el-table-column prop="netWeight" label="Total Net Weight / Master carton_KG" min-width="220px"></el-table-column>
        <el-table-column prop="grossWeight" label="Total Gross Weight / master carton_KG" min-width="220px"></el-table-column>
        <el-table-column prop="masterCartonEan" label="EAN Carton" min-width="150px"></el-table-column>
...
<style>
    ...
    .outbound-table .el-table th.is-leaf,
    .outbound-table .el-table td.is-leaf {
        border: none;
    }
    .outbound-table table.el-table__header {
        padding: 0px !important;
    }
</style>

<script>
import { getOutboundPage, downloadTemplate, importOutbound, downloadData } from "@/api/system/wms/outbound";
...
export default {
  ...
  methods: {
    ...
    // Get outbound list data
    getList() {
      this.loading = true;

      getOutboundPage(this.queryParams)
        .then((response) => {
          this.loading = false;
    ...
    handleExportTemplate() {
      ...
      downloadTemplate()
        .then((response) => {
          this.$download.excel(response, "outbound_template.xlsx");
        })
        .finally(() => {
    ...
    handleExportData() {
      ...
      downloadData()
        .then((response) => {
          this.$download.excel(response, "outbound_data.xlsx");
        })
        .finally(() => {
          this.exportLoading = false;
        });
    ...
    handleImport(fileObj) {
      ...
      this.loading = true;
      importOutbound(formData).then((res) => {
          this.loading = false;
