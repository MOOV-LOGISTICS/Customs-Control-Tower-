...
import{noBookingShippingPage,addRemoveShipping,shippingPoStatus} from '@/api/air/booking'
import { getPoStatusByShippingId } from '@/api/pepco/shipping-order'
import CheckOrderStatusDialog from './check-order-status-dialog.vue';
...
export default {
  ...
  methods: {
    ...
    getNotBookedShipperBooking() {
      ...
      noBookingShippingPage(this.queryParams).then(response => {
    ...
    addPo(row) {
      ...
      getPoStatusByShippingId(row.id).then(response => {
        ...
        const allStatusSame = response.data.length > 0 &&response.data.every(item =>
            item.taskStatus === response.data[0].taskStatus
          );
        if (response.data.length > 0 && (allStatusSame && (response.data[0].taskStatus == 'Carrier Booking')) ) {
          ...
          // dc code, 只有0006和0015能组合，dc不存在会为空字符串随意组合
          if (response.data[0].dc) {
            if (this.selectedTable.length >= 0) {
              const shipment = this.selectedTable.find(item => item.dc === '0006' || item.dc === '0015')
              if (shipment) {
                if (response.data[0].dc !== '0006' && response.data[0].dc !== '0015') {
                  const dcNumber = this.selectedTable.map(item => item.dc).join(',') + ',' + response.data[0].dc
                  this.$message({
                    message: `The DC included in this booking : ${dcNumber}, Only 0006 and 0015 can be booked together!`,
                    type: 'error'
                  })
                  return
                }
    ...
    submit() {
      var data = {
        id: this.shipment.id,
        shippingOrderIds: this.selectedTable.map(item => item.id)
      }
      addRemoveShipping(data).then(res => {
        this.$notify({
          title: 'success',
          message: 'success',
          type: 'success'
