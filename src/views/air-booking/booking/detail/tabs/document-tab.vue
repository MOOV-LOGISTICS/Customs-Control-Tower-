<template>
  <div>
    ...
    <div style="text-align: left; margin-bottom: 15px;">
      <el-button  v-if="shipment.status !==9"  type="primary" icon="el-icon-upload2" @click="showUploadDialog">Upload</el-button>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      title: 'Task Document',
      showBlType: false,
      showMawbBlType: false,
      showHawbBlType: false,
      rules: {
...
//     handler(newVal) {
//       if (newVal && newVal.id) {
//         console.log('Shipment updated:', newVal);
//         this.initComponent(newVal);
//       }
//     },
//     immediate: true // 立即触发一次 watcher
//   }
// },
  ...
  mounted() {
    this.$nextTick(() => {
      if (this.shipment && this.shipment.id) {
        console.log('document shipment', this.shipment);
        this.initComponent(this.shipment);
      }
    });
  },
  methods: {
    initComponent(shipment) {
      this.shipmentId = shipment.id
      this.getOrderFileList()
    },
    getOrderFileList() {
      this.tableLoading = true
      orderFileList({ shipmentId: this.shipmentId, orderHeaderId: null, taskCode: null }).then(res => {
    ...
    submitUploadForm() {
      console.log('submitUploadForm', this.uploadForm)
      ...
      if (this.uploadForm.typeCode === '17' || this.uploadForm.typeCode === '06') {
        this.rules.blType = []
      } else {
        // 否则恢复原始规则
      ...
      this.$refs.uploadFormRef.validate(valid => {
        ...
        if (valid) {
          ...
          this.uploadSet.isUploading = true
          this.uploadForm.taskCode = ''
          this.uploadForm.taskName = ''
    ...
    handleTypeChange(value) {
      ...
      this.uploadForm.typeCode = value
