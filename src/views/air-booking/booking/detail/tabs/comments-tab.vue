<template>
  <div>
    <el-button  v-if="shipment.status !==9" style="margin-bottom: 20px;" type="primary" @click="commentsDialogVisible = true">Add comments
    </el-button>
    <el-empty v-if="bookingCommentList.length === 0"></el-empty>
    <el-timeline>
      <el-timeline-item v-for="(item, i) in bookingCommentList" :key="i" :timestamp="parseTime(item.createTime, '{y}-{m}-{d} {h}:{i}:{s}')" placement="top">
        <el-card style="padding: 15px;">
          <div class="htmldiv" v-html="item.content"></div>
...
import { findCommentsList, commentCreate } from "@/api/air/booking";
...
export default {
  ...
  methods: {
    ...
    getOrderCommentList() {
    console.log('getOrderCommentList',this.shipment)
      this.bookingCommentList = [];
      findCommentsList({ bookingId: this.shipment.id }).then((res) => {
        this.bookingCommentList = res.data;
        this.bookingCommentList.reverse()
      });
    },
    ...
    shipLogCreate() {
      if (this.html.trim() === `<p><br></p>`) {
        ...
        let data = {
          content: this.html,
          commentUser: this.user.email,
          orderType: 1,
          orderId: this.shipment.id,
          commentType:'Comment'
        }
