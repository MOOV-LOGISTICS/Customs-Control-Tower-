...
export default {
  ...
  methods: {
    async openDialog(rows) {
      ...
      console.log('Received rows:', rows.orderHeaderId)
      this.getBookingScore(rows.orderHeaderId)
      this.task = rows

      this.localVisible = true
    },
    getBookingScore(orderHeaderId) {
      getBookingScorePage({ orderHeaderId: orderHeaderId }).then(response => {
        if (response.code !== 0) {
