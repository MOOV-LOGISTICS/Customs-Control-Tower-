  ...
  <el-dialog v-loading="dialogLoading" :title="title" :visible.sync="dialogVisible" width="80%" append-to-body>
      ...
      <el-table :data="tableData" v-loading="tableLoading">
        ...
        <el-table-column prop="blType" label="BL Type" align="center">
          <template slot-scope="scope">
            <span>{{ ['03', '04', '19', '20'].includes(scope.row.typeCode) ? scope.row.blType : '/' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="Action" width="250px" align="center">
          <template #default="scope">
            ...
            <el-link v-if="showEdit&&scope.row.typeCode === '03'">
    ...
    <el-dialog :title="uploadSet.title" :visible.sync="showDialog" width="600px" append-to-body v-loading="uploadSet.loading" :before-close="uploadClose">
      <el-form label-width="150px" ref="uploadFormRef" :model="uploadForm" :show-message="false" :key="formKey">
        ...
        <el-form-item v-if="showBlType" :label="$t('document.status')" prop="blType">
        ...
        <el-form-item v-if="!['15', '18'].includes(uploadForm.typeCode)" :label="$t('document.documentFile')">
          <el-upload
            ref="uploadFileRef"
            :limit="1"
            :accept="['16', '17'].includes(uploadForm.typeCode) ? 'image/*, .jpeg, .jpg, .png, .gif, .doc, .docx, .pdf, .xls, .xlsx' : '.doc, .docx, .pdf, .xls, .xlsx'"
...
import { orderFileList, deleteOrderFile, submitWithoutFile, resaveOrderFile, submitOnlyTask } from '@/api/pepco/order-file'
...
export default {
  ...
  created() {
    console.log('shipping document handler created')
  },
  methods: {
    ...
    submitUploadForm() {
      ...
      if (['15', '18'].includes(this.uploadForm.typeCode)) {
        this.uploadSet.isUploading = true
        const command = {
          milestoneId: this.milestone.id,
          taskCode: this.milestone.taskCode,
          params: { typeCode: this.uploadForm.typeCode, documentNumber: this.uploadForm.documentNumber }
        }
