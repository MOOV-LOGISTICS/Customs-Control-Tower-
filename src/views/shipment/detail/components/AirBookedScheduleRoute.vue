<template>
  <el-card v-if="tableData.length > 0" style="box-shadow: none;padding: 15px;margin-top: 10px;padding-bottom: 0px;">
    <div><b style="font-size: 15px">Air Booking Route</b></div>
    <br />
...
<div style="min-width: 640px;max-width: 2000px;
      ...
      <div>
        <div v-for="(task, index) in tableData" class="self-step4" :key="index" :class="stepClass(index)"
          ...
          <p
            style="display: inline-block; width: 240px; margin: 0px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
            <span class="title">
              <el-popover placement="bottom-start" width="450" popper-class="status-popover-css" trigger="hover"
                ...
                <el-table style="max-height: 300px; overflow: auto;border-bottom: none !important;"
                          :data="timeLogList.filter((item) => index % 2 === 0?item.port == task.pol:item.port == task.pod)">
                  <el-table-column prop="fieldType" label="Event Type">
                    <template v-slot="scope">{{ scope.row.type=='BOOKED_ETD'?'Booked ETD':scope.row.type=='BOOKED_ETA'?'Booked ETA':scope.row.type }}</template>
                  </el-table-column>
                  <el-table-column prop="eventDate" label="Event Date" width="120">
                    <template v-slot="scope">{{ parseUTCTimeYMDHM(scope.row.eventTime) }}</template>
                  </el-table-column>
                  <el-table-column prop="creatorName" max-width="80" label="User">
                    <template v-slot="scope">{{ scope.row.source?scope.row.source:scope.row.creatorName }}</template>
                  </el-table-column>
                ...
                <div slot="reference">
                  <p style="margin:0px">{{ iataMap[task.carrier]?iataMap[task.carrier]:task.carrier }}</p>
                  <div v-if="index % 2 === 0">
                    <p style="margin:0px; text-align: left; position: relative;left: 50px;" v-if="task.bookedEtd">
                      {{ 'Booked ETD:' + parseUTCTimeWithHM(task.bookedEtd) }}
                    </p>
                    <p style="margin:0px; text-align: left; position: relative;left: 50px;" v-if="task.etd">
                      {{ 'ETD:' + parseUTCTimeWithHM(task.etd)  }}
                    </p>
                    <p style="margin:0px; text-align: left; position: relative;left: 50px;" v-if="task.atd">
                      {{ 'ATD:' + parseUTCTimeWithHM(task.atd) }}
                    </p>
                    <p>{{  airportIataMap[task.pol]?airportIataMap[task.pol]:task.pol  }}</p>
                  </div>
                  <div v-if="index % 2 !== 0">
                    <p style="margin:0px; text-align: left; position: relative;left: 50px;" v-if="task.bookedEta">
                      {{ 'Booked ETA:' + parseUTCTimeWithHM(task.bookedEta) }}
                    </p>
                    <p style="margin:0px; text-align: left; position: relative;left: 50px;" v-if="task.eta">
                      {{ 'ETA:' + parseUTCTimeWithHM(task.eta) }}
                    </p>
                    <p style="margin:0px; text-align: left; position: relative;left: 50px;" v-if="task.ata">
                      {{ 'ATA:' + parseUTCTimeWithHM(task.ata) }}
                    </p>
                    <p>{{  airportIataMap[task.pod]?airportIataMap[task.pod]:task.pod  }}</p>
                  </div>
                </div>
              </el-popover>
              
              <el-popover placement="bottom-start" width="600" popper-class="status-popover-css" trigger="hover">
                <p v-if="index % 2 == 0 && task.transportMode === 'SEA'" slot="reference" :class="{ 'highlight': false }"
                ...
                <p v-if="index % 2 == 0 && task.transportMode === 'AIR'" slot="reference" :class="{ 'highlight': false }"
                   style="margin: 0px; width: 150px; position: absolute; left: 180px; top: -20px; white-space: normal; cursor: pointer;">
                  {{ iataMap[task.carrier]?iataMap[task.carrier]:task.carrier }}<br>{{ task.flight }}
                </p>
              </el-popover>
            </span>
          </p>
      ...
      <div style="padding-left: 200px;">
        ...
        <el-button v-if="shipment.status !== 9" @click="showUpdateSeaAirBookingNumber" type="success" style="margin-left: 10px;">Update Booking Number</el-button>
      </div>
    </div>


          <add-event-time-dialog ref="addEventTimeDialog" :visible.sync="showAddEventDialog" :data="eventData" :shipment="shipment"
            @submit="handleSubmit"></add-event-time-dialog>
          <update-event-dialog ref="updateEventDialog" :visible.sync="showUpdateEventDialog" :eventCode="eventCode"
            :form="eventForm" @submit="handleSubmitEvent"></update-event-dialog>
          <update-sea-air-booking-number-dialog ref="updateSeaAirBookingNumberDialog" :visible.sync="showUpdateSeaAirBookingNumberDialog" :data="updateSeaAirBookingNumberData" @submit="handleUpdateSeaAirBookingNumber"></update-sea-air-booking-number-dialog>
  </el-card>
</template>

<script>

import addEventTimeDialog from "@/views/shipment/detail/components/add-event-time-dialog.vue";
import updateEventDialog from "@/views/shipment/detail/components/update-event-dialog.vue";
...
import { seaAirTimeLogListByShipmentId } from '@/api/seaair/seaair';

export default {
  name: 'AirBookedScheduleRoute',
  components: { addEventTimeDialog, updateEventDialog, updateSeaAirBookingNumberDialog },
  ...
  methods: {
    getSeaAirTimeLog(){
      seaAirTimeLogListByShipmentId({ id: this.shipmentId }).then((res) => {
        console.log(res)
    ...
    */
    ...
    async fetchData() {
      ...
      this.loading = true;
      try {
        const response = await seaAirBookingRouterByBookingId(this.shipment.id);
        ...
        console.error('Failed to load air router data:', error);
        this.$message.error('Failed to load air router data');
        this.tableData = [];
      } finally {
        this.loading = false;
      }
    ...
    stepClass(index) {
      return index % 2 === 0 ? 'step-active' : 'step-inactive';
    },
    ...
    parseUTCTime(time, pattern) {
      ...
      // 3. 替换format占位符（核心：让format生效）
      const timeStr = format.replace(/{([ymdhis])+}/g, (_, key) => formatObj[key] || '00');
      return timeStr
    },

    parseUTCTimeYMDHM(time) {
      return this.parseUTCTime(time, '{y}-{m}-{d} {h}:{i}')
    },
    ...
    showUpdateSeaAirBookingNumber() {
      ...
      this.updateSeaAirBookingNumberData = {
        airBookingNumber: this.shipment.seaAirBookingNumber || '',
        shipmentId: this.shipment.id,
        airConfirmationDate: this.shipment.seaAirBookingConfirmation || null
      };
