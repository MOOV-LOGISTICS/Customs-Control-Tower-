...
export default {
  ...
  methods: {
    ...
    fetchDetail() {
      ...
      shippingMilestoneAggregate(this.task.id).then(res => {
        console.log('preAlertDetail', res.data)
        const { shipment, mbl } = res.data
        this.shipment = Object.assign({}, shipment.shipment)
        console.log('shipment111', shipment)
        this.mblFile = Object.assign({}, mbl)
        const { shippingOrders, shipmentItems, clpTasks, shipmentClpList, shipmentClpDetailList } = res.data.shipment
        ...
        this.$nextTick(() => {
          ...
          this.$refs.uploadFileRef.initComponent(this.task, this.task.shipmentId, true)
        })
        this.dialogLoading = false
      })
    },
    submitPreAlertTask() {
      ...
      this.submitForm.shipmentId = this.shipment.shipment.id
