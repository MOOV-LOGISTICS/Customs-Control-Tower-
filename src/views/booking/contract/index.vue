  ...
  <div class="app-container">
    <div class="contract-list">
      ...
      <el-card style="padding:20px 0px 10px 20px;background-color: #fff;">
        <el-row>
          <el-col :span="5">
            <el-select filterable v-model="queryParams.customerId" size="small" @change="handleQuery"
                       clearable placeholder="Customer Name">
              <el-option
                v-for="dept in customerOptions"
                :value="dept.id"
                :key="dept.id"
                :label="dept.name"
              ></el-option>
      ...
      <el-card style="padding:20px;background-color: #fff;">
        ...
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch"
          ...
          <el-form-item style=" background-color: transparent;">
            ...
            <el-button @click="handleAdd" type="primary" plain>Create<i class="el-icon-plus el-icon--right"
                                                                        v-hasPermi="['booking:contract:create']"></i>
            </el-button>
          </el-form-item>
          <div style="display: contents;">
            <el-button @click="openRegionAdd" type="primary" plain v-hasPermi="['booking:contract:create']"
                       style="    float: right;">
              Create Region<i class="el-icon-plus el-icon--right"></i></el-button>
          </div>
        </el-form>
          ...
          <el-table-column align="center" class-name="small-padding fixed-width">
            <template v-slot="scope">
              <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                         v-hasPermi="['booking:contract:delete']">delete
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-dialog :title="title" :visible.sync="open" width="100%"
        ...
        <el-form ref="form" :model="form" size="mini" :rules="rules" label-width="120px" v-loading="formLoading">
          ...
          <el-row :gutter="20" class="contract-css">
            ...
            <el-col :span="3">
              ...
              <el-upload
                ...
                ref="upload" :headers="upload.headers"
                :action="upload.url"
                ...
                :on-change="(file, files) => {handleFileChangeThen(file, files, ()=>{this.form.fileList = files}, () => {this.form.fileList = []})}"
                :auto-upload="false">

                <svg style="cursor: pointer;" width="30" height="30" viewBox="0 0 48 48" fill="none"
                     xmlns="http://www.w3.org/2000/svg">
                  <mask id="icon-21f8f28d7f10b972" maskUnits="userSpaceOnUse" x="0" y="0" width="48" height="48"
                        style="mask-type: alpha">
                    <path d="M48 0H0V48H48V0Z" fill="#004F7C"/>
                  </mask>
                  <g mask="url(#icon-21f8f28d7f10b972)">
                    <path d="M6 24.0083V42H42V24" stroke="#004F7C" stroke-width="4" stroke-linecap="round"
                          stroke-linejoin="round"/>
                    <path d="M33 15L24 6L15 15" stroke="#004F7C" stroke-width="4" stroke-linecap="round"
                          stroke-linejoin="round"/>
                    <path d="M23.9917 32V6" stroke="#004F7C" stroke-width="4" stroke-linecap="round"
                          stroke-linejoin="round"/>
                  </g>
                </svg>
          ...
          <el-row :gutter="20" style="margin-bottom: 15px" class="contract-css">
            ...
            <el-col :span="4" :offset="16">
              <svg style="cursor: pointer;" @click="handleImport" width="30" height="30" viewBox="0 0 48 48" fill="none"
                   xmlns="http://www.w3.org/2000/svg">
                <mask id="icon-21f8f28d7f10b972" maskUnits="userSpaceOnUse" x="0" y="0" width="48" height="48"
                      style="mask-type: alpha">
                  <path d="M48 0H0V48H48V0Z" fill="#004F7C"/>
              ...
              >
                <el-upload
                  :show-file-list="false" :on-change="beforeUploadContainers" :auto-upload="false"
                  accept=".xlsx,.xls"
                  drag
                >
          ...
          <!--          <div v-if="[1, 3, 4, 5, 6, 7].includes(showVessel)">-->
          <!--            <el-row :gutter="20" style="margin-bottom: 15px">-->
          <!--              <el-col :span="4">-->
          <!--                Region and ports-->
          <!--              </el-col>-->
          <!--              <el-col :span="4">-->
          <!--                <el-button @click="addRegionPortRow" type="primary">Add Region and ports</el-button>-->
          <!--              </el-col>-->
          <!--            </el-row>-->

          <!--            <el-row :gutter="20" style="margin-bottom: 10px">-->
          <!--              <el-col :span="4">Region</el-col>-->
          <!--              <el-col :span="4">POL</el-col>-->
          <!--            </el-row>-->
          <!--            <div style="margin-bottom: 5px"-->
          <!--                 v-for="(item, i) in (typeof(form.regionPorts) == 'string' ?  JSON.parse(this.form.regionPorts): form.regionPorts)"-->
          <!--                 :key="i+'regionPorts'"-->
          <!--                 class="contract-css pol">-->
          <!--              <el-row :gutter="20">-->
          <!--                <el-col :span="4">-->
          ...
          <!--                </el-col>-->
          <!--                <el-col :span="18">-->
          <!--                  <el-select filterable multiple  v-model="item.pol"-->
          <!--                             class="custom-select">-->
          <!--                    <el-option-->
          ...
          <!--                  </el-select>-->
          <!--                </el-col>-->
          <!--                <el-col :span="2" v-if="i >= 1">-->

          <!--                  <el-button-->
          <!--                    size="mini"-->
          <!--                    type="text"-->
          <!--                    icon="el-icon-delete"-->
          <!--                    @click="deleteRegionPortRow(i)"-->
          <!--                  >{{ $t("search.delete") }}-->
          <!--                  </el-button>-->
          ...
          <el-row :gutter="20" style="padding-bottom: 10px;">
            ...
            <el-col :span="4" v-show="title == 'Update'">
              ...
              <el-date-picker
                v-model="queryItemParams.allocationYear"
                type="year"
                value-format="yyyy"
                @change="queryItemParams.pageNo =1 ;getContractItemPage()"
                placeholder="Allocation Year">
              </el-date-picker>
            </el-col>
            <el-col :span="4" v-show="title == 'Update'">
              <el-select v-model="queryItemParams.allocationWeek" placeholder="Allocation Week"
                         @change="getContractItemPage" multiple filterable>
                <el-option
          ...
          <el-table v-loading="subItemLoading"
                    ...
                    style="width: 100%">
            <el-table-column
              prop="allocationYear" min-width="100px"
              label="Allocation Year">
              <template slot-scope="scope">
                <el-date-picker
                  v-model="scope.row.allocationYear"
                  type="year"
            ...
            <el-table-column
              prop="allocationMonth" min-width="100px"
              label="Allocation Month" v-if="showVessel == 2" key="0">
              <template slot-scope="scope">
                <el-date-picker
                  popper-class="hlcu-month"
                  v-model="scope.row.allocationMonth"
                  type="month"
                  style="width: 150px"
                  placeholder="Month"
                  format="MMM"
            ...
            <el-table-column
              prop="allocationWeek" min-width="100px"
              label="Allocation Week" v-if="[1, 3, 4, 5, 6, 7].includes(showVessel)" key="1">
              <template slot-scope="scope">
            ...
            <el-table-column
              prop="contractType" min-width="140px"
              label="Contract Type" v-if="[1, 3, 4 ].includes(showVessel)" key="10">
              <template slot-scope="scope">
            ...
            <el-table-column min-width="130px" prop="region" label="Region"
                             v-if="[1, 3, 4, 5, 6, 7].includes(showVessel)" key="2">
              <template slot-scope="scope">
                <el-select filterable v-model="scope.row.region" clearable class="custom-select">
                  <el-option
                    v-for="(item, index) in contractRegionList"
                    :key="index"
                    :label="item.region"
                    :value="item.id+''"
                  ></el-option>
            ...
            <el-table-column prop="pod" min-width="140px" label="POD" v-if="[1,  4].includes(showVessel)" key="3">
              <template slot-scope="scope">
            ...
            <el-table-column min-width="180px" prop="vessel" label="Vessel"
                             v-if="showVessel == 2 || showVessel == 3  || showVessel == 7">
              <template slot-scope="scope">
                <el-select
            ...
            <el-table-column min-width="120px" prop="voyage" label="Voyage"
                             v-if="showVessel == 2 || showVessel == 3  || showVessel == 7">
              <template slot-scope="scope">
            ...
            <el-table-column
              prop="contractualTeu" min-width="150px"
              label="Contractual (TEU)">
              <template slot-scope="scope">
            ...
            <el-table-column
              prop="allocationTeu" min-width="150px"
              label="Confirmed (TEU)">
              <template slot-scope="scope">
            ...
            <el-table-column
              prop="extarAllocationTeu" min-width="165px"
              label="Confirmed Extra (TEU)">
              <template slot-scope="scope">
                <el-input-number :controls="false" :precision="1" :max="9999999"
                                 v-model="scope.row.extarAllocationTeu"/>
              </template>
            </el-table-column>
            <el-table-column
              prop="remark" min-width="100px"
              label="Remark">
              <template slot-scope="scope">
                <el-input
                  type="textarea"
            ...
            <el-table-column label="Actions" min-width="150px">
              <template slot-scope="scope">
                <el-button v-show="scope.row.id != null"
                           size="mini"
                           type="text"
                           icon="el-icon-edit"
                           @click="updateContractItem(scope.row)"
                >{{ $t("booking.update") }}
                </el-button>
                <el-button
                  size="mini"
          ...
          <el-form-item label="Comments" prop="remark" label-width="90px">
            <el-input v-model="form.remark" type="textarea" rows="3"
                      placeholder="Notes for changes in allocation or special requests for allocation. "/>
          </el-form-item>

        </el-form>
        <div slot="footer" class="dialog-footer">
          <pagination v-show="subItemTotal > 0" :total="subItemTotal" :page.sync="queryItemParams.pageNo"
                      :limit.sync="queryItemParams.pageSize" class="itemPageCss"
                      @pagination="getContractItemPage"/>
    ...
    <el-dialog title="Region" :visible.sync="regionVisible" width="100%"
               @open="openRegion"
               fullscreen
               destroy-on-close>
      ...
      <el-form :model="queryParamsRegion" ref="queryFormRegion" size="small" :inline="true" label-width="68px">
        <el-form-item label="Region" prop="region">
          <el-input v-model="queryParamsRegion.region" placeholder="Region" clearable
                    @keyup.enter.native="handleRegionQuery"
                    @blur="queryParamsRegion.region = $event.target.value.trim()"/>
        </el-form-item>
        <el-form-item>
          <el-button @click="handleRegionQuery" type="primary">Search<i class="el-icon-search el-icon--right"></i>
          </el-button>

          <el-button @click="handleRegionAdd" type="primary" plain>Create<i class="el-icon-plus el-icon--right"></i>
          </el-button>
      ...
      <el-table v-loading="regionLoading" :data="regionList">
        ...
        <el-table-column label="" align="center" class-name="small-padding fixed-width" min-width="220">
          <template v-slot="scope">
            <el-button size="mini" type="text" icon="el-icon-edit" @click="handleRegionUpdate(scope.row)"
            >Update
            </el-button>
            <el-button size="mini" type="text" icon="el-icon-delete" @click="handleRegionDelete(scope.row)"
            >Delete
            </el-button>
          </template>
        </el-table-column>
      ...
      <!-- 分页组件 -->
      <pagination v-show="regionTotal > 0" :total="regionTotal" :page.sync="queryParamsRegion.pageNo"
                  :limit.sync="queryParamsRegion.pageSize"
                  @pagination="getRegionList"/>

      <!-- 对话框(添加 / 修改) -->
      <el-dialog :title="regionTitle" :visible.sync="regionOpen" width="75%" v-dialogDrag append-to-body>
        <el-form ref="regionForm" :model="regionForm" :rules="rules" label-width="80px"
                 :show-message="false">
          ...
          <el-form-item label="Ports" required prop="ports">
            <el-select filterable multiple v-model="regionForm.ports"
                       class="custom-select-2" placeholder="Ports">
              <el-option
...
<script>
  ...
  import { createContract, updateContract, deleteContract, downloadContractItemTemplate, getWeeksByContractId,
    getContract, getContractPage, exportContractExcel,getContractItemPage, deleteContractItem,updateContractItem
  ...
  import { createContractRegion, updateContractRegion, deleteContractRegion, getContractRegion,
   getContractRegionPage, getContractRegionList } from "@/api/booking/contractRegion";
  ...
  export default {
    ...
    data() {
      return {
        ...
        upload: {
          ...
          // 上传的地址
          url: process.env.VUE_APP_BASE_API + '/admin-api/shipment/contract/uploadContractFile'
        },
        checkLoading :false,
    ...
    methods: {
      ...
      handleRegionDelete(row) {
        const id = row.id;
        this.$modal.confirm('Are you sure you want to delete it? ').then(function() {
        ...
        this.file2Xce(file).then(tabJson => {
            if (tabJson && tabJson.length > 0 && tabJson[0].sheet.length > 0) {
                ...
                listTable.forEach((item, index) => {
                    ...
                    var regionId = null;
                    if(item["POD"]){
                      var obj = this.podList.find((it) => (it.code == item["POD"].trim() || it.name == item["POD"].trim()))
                      if(obj){
                        podCode = obj.code
                      }
                    }
                    ...
                    if(item["Region"]){
                      var data = this.contractRegionList.find(it =>  it.region == item["Region"].trim())
                      if(data){
                        regionId = data.id+''
                      }
                    }
                    ...
                    this.subItemData.push({
                      ...
                      region: regionId,
                      pod: podCode,
                      vessel: item["Vessel"] ? item["Vessel"].trim() : null,
                      voyage: item["Voyage"] ? item["Voyage"].trim() : null,
                      vessel2: item["Vessel2"] ? item["Vessel2"].trim() : null,
                      voyage2: item["Voyage2"] ? item["Voyage2"].trim() : null,
                      service: item["Service"] ? item["Service"].trim() : null,
      ...
      updateContractItem(row){
        ...
        this.$modal.confirm('Are you sure you want to update it? ').then(function() {
      ...
      handleExport() {
        ...
        this.$modal.confirm('是否确认导出所有booking_contract数据项?').then(() => {
