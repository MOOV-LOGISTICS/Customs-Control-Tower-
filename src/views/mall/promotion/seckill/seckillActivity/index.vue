    ...
    <div class="app-container" style="width:90%;margin:0 auto;">
        ...
        <el-form :show-message="false"	 	 size='mini' :model="queryParams" ref="queryForm"  :inline="true" v-show="showSearch"
            ...
            <el-form-item label="参与场次" prop="timeId">
                <el-select v-model="queryParams.timeId" placeholder="请选择参与场次" clearable >
        ...
        <el-row :gutter="10" class="mb8">
            ...
            <el-col :span="1.5">
                <el-button size="mini"  type="primary" plain icon="el-icon-menu"  @click="openSeckillTime"
            ...
            <el-table-column label="参与场次" prop="timeIds" width="250">
                <template v-slot="scope">
                    <span v-for="item in seckillTimeList" :key="item.id"
                        ...
                        <el-tag style="margin:4px;" >{{ item.name }}</el-tag>
                    </span>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template v-slot="scope">
                    <el-button size="mini"   type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                        v-hasPermi="['promotion:seckill-activity:update']">{{ $t('search.modify') }}</el-button>
                    ...
                    <el-button size="mini"   type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                        v-hasPermi="['promotion:seckill-activity:delete']">{{ $t('search.delete') }}</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!-- 分页组件 -->
        ...
        <el-dialog :title="title" :visible.sync="open" width="1200px" v-dialogDrag append-to-body>
            <el-form :show-message="false"	 	 size='mini' ref="form" :model="form" :rules="rules" label-width="120px">
                ...
                <el-form-item label="场次选择">
                    <el-select v-model="form.timeIds" placeholder="请选择参与场次" clearable  multiple filterable
                        style="width: 880px">
                        <el-option v-for="item in seckillTimeList" :key="item.id" :label="item.name" :value="item.id">
                ...
                <el-form-item label="商品选择">
                    <el-select v-model="form.skuIds" placeholder="请选择活动商品" clearable  multiple filterable
                    ...
                    <el-row :gutter="30">
                        <el-button size="mini"  type="primary"  @click="batchEditProduct('limitBuyCount')">限购</el-button>
                        <el-button size="mini"  type="primary"  @click="batchEditProduct('seckillPrice')">秒杀价</el-button>
                        <el-button size="mini"  type="primary"  @click="batchEditProduct('seckillStock')">秒杀库存</el-button>
                    </el-row>
                    <el-table v-loading="loading" ref="productsTable" :data="form.products">
                        ...
                        <el-table-column label="限购(0为不限购)" align="center" width="150">
                            <template v-slot="scope">
                                <el-input-number v-model="scope.row.limitBuyCount"  :min="0" :max="10000">
                                </el-input-number>
                            </template>
                        </el-table-column>
                        <el-table-column label="秒杀价(元)" align="center" width="150">
                            <template v-slot="scope">
                                <el-input-number v-model="scope.row.seckillPrice"  :precision="2" :min="0"
                                    :max="10000">
                                </el-input-number>
                            </template>
                        </el-table-column>
                        <el-table-column label="秒杀库存" align="center" width="150" prop="seckillStock">
                            <template v-slot="scope">
                                <el-input-number v-model="scope.row.seckillStock"  :min="0" :max="10000">
                                </el-input-number>
                            </template>
                        </el-table-column>
                        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
