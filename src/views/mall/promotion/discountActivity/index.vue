    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button size="mini"  type="primary" plain icon="el-icon-plus"  @click="handleAdd"
                   v-hasPermi="['promotion:discount-activity:create']">{{ $t('search.addNew') }}</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini"   type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     ...
                     v-hasPermi="['promotion:discount-activity:update']">{{ $t('search.modify') }}</el-button>
          ...
          <el-button size="mini"   type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-if="scope.row.status === PromotionActivityStatusEnum.CLOSE.type"
                     v-hasPermi="['promotion:discount-activity:delete']">{{ $t('search.delete') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
    ...
    <el-dialog :title="title" :visible.sync="open" width="1000px" v-dialogDrag append-to-body>
      <el-form :show-message="false"	 	 size='mini' ref="form" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="商品选择">
          <el-select v-model="form.skuIds" placeholder="请选择活动商品" clearable 
          ...
          <el-table v-loading="loading" :data="form.products">
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
              <template v-slot="scope">
                <el-button size="mini"   type="text" icon="el-icon-delete" @click="removeFormSku(scope.row.skuId)">{{ $t('search.delete') }}</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-form-item>
