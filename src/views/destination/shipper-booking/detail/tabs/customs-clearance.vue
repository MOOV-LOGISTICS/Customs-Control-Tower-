<template>
  <div style="margin-top: -25px; ">
    ...
    <el-table :data="logList">
      ...
      <el-table-column prop="submitDate" label="Submit Date" align="center" width="200px">
        <template #default="scope">{{ formatUserLocalTime(scope.row.paramsDTO.submitDate, scope.row.timeZone, scope.row.timeZoneOffset) || '-'}}</template>
      </el-table-column>
      <el-table-column prop="finishDate" label="Finish Date" align="center" width="200px">
        <template #default="scope">{{ formatUserLocalTime(scope.row.paramsDTO.finishDate, scope.row.timeZone, scope.row.timeZoneOffset) || '-'}}</template>
      </el-table-column>

      <el-table-column prop="updater" label="Update User" align="center" width="200px">
        <template #default="scope">{{ getUserName(scope.row.updater) }}</template>
      </el-table-column>
      <el-table-column prop="updateTime" label="Update Time" align="center" width="200px">
