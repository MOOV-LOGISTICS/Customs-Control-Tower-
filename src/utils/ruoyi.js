...
export function parseUTCTime(time, pattern) {
  if (arguments.length === 0 || !time) {
    return null
  }
  const format = pattern || '{y}-{m}-{d}'
...
export function parseUTCWithTime(time, pattern) {
  if (arguments.length === 0 || !time) {
    return null;
  }
...
export function parseUTCTime2(time, pattern) {
  if (arguments.length === 0 || !time) {
    return null
  }
  const format = pattern || '{y}-{m}-{d}'
  ...
  const formatMap = {
    ...
    i: parts.find(p => p.type === 'minute')?.value,
    s: parts.find(p => p.type === 'second')?.value,
  };
  const time_str = format.replace(/{([ymdhisa])+}/g, (result, key) => {
    const value = formatMap[key];
    if (value !== undefined && value !== null) {
      const numValue = parseInt(value, 10); // 转为数字
      if (!isNaN(numValue) && numValue < 10) {
        return "0" + numValue; // 补零
      }
      return value; // 直接返回原字符串（已经是两位数）
    }
    return "00"; // 缺失字段默认填充 00
  });
  return time_str;
}
export function parseUTCTimeWithHM(time, pattern) {
  if (arguments.length === 0 || !time) {
    return null
  }
  const format = pattern || '{y}-{m}-{d}'
...
export function parseUTCTime1(time, pattern) {
  if (arguments.length === 0 || !time) {
    return null
  }
  const format = pattern || '{y}-{m}-{d}'
  ...
  if (typeof time === 'object') {
    ...
    date = addTimezoneHours(new Date(time), 8 * 2); // 乘以2因为每个时区大约等于2小时
  }
  const formatObj = {
...
function addTimezoneHours(date, hours) {
