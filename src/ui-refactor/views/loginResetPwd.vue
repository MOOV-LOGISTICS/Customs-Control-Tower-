<template>
  ...
  >
    <el-form-item v-if="useType === 'new'" :label="$t('userInfo.firstName')" prop="firstName">
      <el-input v-model="user.firstName" placeholder="Enter First Name" />
    </el-form-item>
    <el-form-item v-if="useType === 'new'" :label="$t('userInfo.familyName')" prop="familyName">
      <el-input v-model="user.familyName" placeholder="Enter Family Name" />
    </el-form-item>
          ...
          },
          {
            ...
            validator: (rule, value, callback) => {
              ...
              if (!passwordreg.test(value)) {
                callback(
                  new Error(
                    "Business Phone must contain only digits and hyphens"
                  )
                );
              } else {
                callback();
    ...
    submit() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          initPassword(data).then((response) => {
